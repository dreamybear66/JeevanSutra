      ...
      <div className="content-layout">
        ...
        <main className="main-area">
          {activeNav === 'admin' && currentUser.role === 'doctor' ? (
            ...
            <>
              <div className="glass-panel patient-selector-bar" style={{ display: 'flex', gap: '12px', marginBottom: '20px', alignItems: 'center', padding: '16px', borderRadius: 'var(--radius)', background: 'var(--bg-glass)', backdropFilter: 'blur(10px)', border: '1px solid var(--border-glass)', boxShadow: 'var(--shadow-sm)' }}>
                <span style={{ fontWeight: '700', color: 'var(--text-secondary)' }}>Patient Profile:</span>
                <select 
                  className="patient-select" 
                  value={selectedScenario || ''} 
                  onChange={(e) => { setSelectedScenario(e.target.value); setReport(null) }}
                  style={{ flex: 1, padding: '10px 14px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', background: 'var(--bg-white)', outline: 'none', fontFamily: 'inherit', fontSize: '0.85rem' }}
                >
